# RFC Index

RFC numbers are permanent and must never be reused.

## Active RFCs

| RFC | Title | Status |
|---|---|---|
| [RFC-0000](RFC-0000-Event-Bus-and-Execution-Model.md) | Event Bus and Execution Model | Draft |
| [RFC-0001](RFC-0001-Core-Architecture.md) | Core Architecture | Stub |
| [RFC-0002](RFC-0002-Collections-and-Schema.md) | Collections and Schema | Draft |
| [RFC-0003](RFC-0003-Functions-Engine.md) | Functions Engine | Draft |
| [RFC-0004](RFC-0004-Jobs-Engine.md) | Jobs Engine | Draft |
| [RFC-0005](RFC-0005-Authentication-and-Permissions.md) | Authentication and Permissions | Draft |
| [RFC-0006](RFC-0006-Realtime-Engine.md) | Realtime Engine | Stub |
| [RFC-0007](RFC-0007-Storage-System.md) | Storage System | Stub |
| [RFC-0008](RFC-0008-CLI-and-Project-Structure.md) | CLI and Project Structure | Stub |
| [RFC-0009](RFC-0009-SDK-Design.md) | SDK Design | Stub |
| [RFC-0010](RFC-0010-Packaging-and-Deployment.md) | Packaging and Deployment | Stub |

## Reserved RFC Numbers

| RFC | Title |
|---|---|
| RFC-0011 | Admin Dashboard |
| RFC-0012 | Migration Engine |
| RFC-0013 | Logging and Observability |
| RFC-0014 | Backup and Restore |
| RFC-0015 | Plugin System |
| RFC-0016 | Security Model |
| RFC-0017 | Testing Strategy |
| RFC-0018 | Versioning and Upgrade Policy |

## Reading Order

1. RFC-0000
2. RFC-0001
3. RFC-0002 through RFC-0010
4. [ARCHITECTURE.md](../ARCHITECTURE.md)
5. [TECH-STACK.md](../TECH-STACK.md)
